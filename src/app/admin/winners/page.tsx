...
export default async function AdminWinnersPage() {
  ...
  const { data: allWinners } = await supabase
    .from("draw_winners")
    .select(
      "*, profiles(email), draws(id, winning_numbers, total_pool, execution_date)",
    )
    .order("created_at", { ascending: false });

  const pending = allWinners?.filter((w) => w.status === "pending") || [];
  const approved =
    allWinners?.filter((w) => w.status === "approved" || w.status === "paid") ||
    [];
    ...
    <div className="p-4 sm:p-8 space-y-10 animate-in fade-in">
      <div>
        ...
        <p className="text-muted-foreground">
          All winners from published draws. Verify submitted scores against the
          draw results.
        </p>
      </div>
      ...
      {!allWinners || allWinners.length === 0 ? (
        <div className="glass-panel p-16 text-center rounded-2xl border-dashed border-white/10">
          ...
          <p className="text-xs text-muted-foreground/60 mt-1">
            Publish a draw to generate winners automatically.
          </p>
        </div>
      ) : (
        <div className="space-y-4">
          {allWinners.map((winner) => {
            ...
            const entries: {
              ...
              date: string;
              created_at: string;
            }[] = winner.matched_score_entries || [];
            const winningNums: number[] = draw?.winning_numbers || [];

            return (
              <div
                key={winner.id}
                className="glass-panel p-6 rounded-2xl border-white/10 space-y-5"
              >
                {/* Header */}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      <span className="flex items-center gap-1 text-xs text-muted-foreground">
                        <Hash className="w-3 h-3" /> Draw{" "}
                        <span className="font-mono text-white/60">
                          {winner.draw_id?.slice(0, 8).toUpperCase()}
                        </span>
                      </span>
                      {draw?.execution_date && (
                        <span className="flex items-center gap-1 text-xs text-muted-foreground">
                          <Calendar className="w-3 h-3" />{" "}
                          {new Date(draw.execution_date).toLocaleDateString()}
                        </span>
                      )}
                    </div>
                  </div>
                  <span
                    className={`text-xs px-2.5 py-1 rounded-full border font-semibold uppercase tracking-wide ${statusBadge(winner.status)}`}
                  >
                    {winner.status}
                  </span>
                </div>
                  ...
                  <div className="bg-white/5 rounded-lg px-3 py-2 md:col-span-2">
                    ...
                    <div className="flex gap-1.5 flex-wrap">
                      {winningNums.map((n, i) => (
                        <span
                          key={i}
                          className="w-7 h-7 rounded-full bg-primary/20 border border-primary/40 text-primary text-xs font-bold flex items-center justify-center"
                        >
                          {n}
                        </span>
                      ))}
                    ...
                    return (
                      <div className="bg-black/20 border border-dashed border-white/10 rounded-xl p-4 text-center">
                        <p className="text-xs text-muted-foreground">
                          ⏳ Awaiting proof upload from user
                        </p>
                      </div>
                    );
                  const isImage = /\.(png|jpe?g|gif|webp|avif|svg)$/i.test(
                    proof,
                  );
                ...
                <div className="bg-black/30 rounded-xl border border-white/5 overflow-hidden">
                  <div className="px-4 py-2 border-b border-white/5 flex items-center gap-2">
                    ...
                    <p className="text-sm font-semibold">
                      Submitted Scores (Verification Record)
                    </p>
                    <p className="text-xs text-muted-foreground ml-auto">
                      {entries.length} of 5 entries
                    </p>
                  </div>
                  {entries.length === 0 ? (
                    <div className="px-4 py-6 text-center">
                      <p className="text-xs text-muted-foreground">
                        Score entries were not captured for this draw. Only
                        future publishes will include them.
                      </p>
                    </div>
                  ) : (
                        ...
                        <tbody className="divide-y divide-white/5">
                          {entries.map((entry, i) => {
                            ...
                            return (
                              <tr
                                key={i}
                                className={isMatch ? "bg-primary/5" : ""}
                              >
                                <td className="px-4 py-3">
                                  <span
                                    className={`inline-flex w-8 h-8 rounded-full items-center justify-center font-bold text-sm ${isMatch ? "bg-primary/20 text-primary border border-primary/40" : "bg-white/10 text-white/60"}`}
                                  >
                                    {entry.score}
                                  </span>
                                </td>
                ...
                {winner.status === "pending" && (
                  <div className="flex flex-col sm:flex-row gap-3">
                    ...
                    >
                      <button
                        type="submit"
                        className="bg-emerald-500 hover:bg-emerald-600 text-white font-bold px-5 py-2 rounded-lg flex items-center gap-2 transition-colors text-sm"
                      >
                    ...
                    >
                      <button
                        type="submit"
                        className="bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20 font-bold px-5 py-2 rounded-lg flex items-center gap-2 transition-colors text-sm"
                      >
