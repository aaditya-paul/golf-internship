...
export default function AdminUserList({
  ...
  async function handleDeleteScore(scoreId: string, userId: string) {
    if (
      !confirm(
        "Are you sure you want to delete this score? This may affect their lottery ticket.",
      )
    )
      return;
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    Charity: {profile.charity?.name || "None"} (
              ...
              >
                <div className="flex items-center gap-2">
                  <Activity
                    className={`w-4 h-4 ${profile.subscription_status === "active" ? "text-primary" : "text-muted-foreground"}`}
                  />
                  <select
                  ...
                  >
                    ...
                    <option value="past_due" className="bg-black text-white">
                      Past Due
                    </option>
                    <option value="canceled" className="bg-black text-white">
                      Canceled
                    </option>
                  </select>
                </div>
            ...
            {isExpanded && (
              <div className="p-4 bg-black/40 border-t border-white/5">
                <h4 className="text-sm font-semibold mb-3">
                  Recent Scores ({profile.scores?.length || 0})
                </h4>
                <div className="space-y-2">
                  {profile.scores?.length === 0 ? (
                    ...
                    profile.scores?.map((score: any) => (
                      <div
                        key={score.id}
                        className="flex justify-between items-center bg-white/5 px-3 py-2 rounded-lg gap-3"
                      >
                        ...
                        <button
                          onClick={() =>
                            handleDeleteScore(score.id, profile.id)
                          }
