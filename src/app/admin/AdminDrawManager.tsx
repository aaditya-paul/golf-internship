...
import { PlayCircle, CheckCircle, Loader2, TrendingUp, TrendingDown, Shuffle, Info, AlertTriangle } from 'lucide-react'
...
const MODES = [
  { value: 'most_frequent', label: 'Most Frequent Scores', icon: TrendingUp, desc: 'Picks the 5 numbers players submit most often — higher community engagement.' },
  { value: 'least_frequent', label: 'Least Frequent Scores', icon: TrendingDown, desc: 'Picks the 5 numbers submitted least — harder to win, bigger suspense.' },
  { value: 'random', label: 'Pure Random (RNG)', icon: Shuffle, desc: 'Completely random 5 numbers, 1–45. No bias.' },
] as const

export default function AdminDrawManager({ activeDraws }: { activeDraws: any[] }) {
    const now = new Date()
    const [loading, setLoading] = useState(false)
    const [mode, setMode] = useState<'most_frequent' | 'least_frequent' | 'random'>('most_frequent')
    ...
    async function handlePublish(id: string) {
        if (!confirm('Publish this draw? This is irreversible — all users will be notified immediately.')) return
        setLoading(true)
        setError(null)
    ...
    return (
        <div className="space-y-8">
                ...
                {fetchingPool ? (
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        ...
                        <div className="bg-black/30 p-3 rounded-xl">
                            ...
                            <p className="font-bold text-lg">${poolData.pricePerSub.toFixed(2)}</p>
                        </div>
                        <div className="bg-black/30 p-3 rounded-xl">
                            <p className="text-muted-foreground text-xs mb-1">Draw Allocation (20%)</p>
                            <p className="font-bold text-lg">${poolData.basePool.toFixed(2)}</p>
                        </div>
                        <div className="bg-primary/10 border border-primary/30 p-3 rounded-xl">
                            <p className="text-primary text-xs mb-1 font-semibold">
                                Final Pool {poolData.carryover > 0 ? `(+$${poolData.carryover.toFixed(2)} rollover)` : ''}
                            </p>
                            <p className="font-bold text-2xl text-primary">${poolData.finalPool.toFixed(2)}</p>
                        </div>
                    </div>
                ) : null}
                    ...
                    <div className="grid grid-cols-3 gap-3 text-xs text-center">
                        {([5, 4, 3] as const).map(tier => (
                            <div key={tier} className="bg-white/5 rounded-lg p-2 border border-white/5">
                                ...
                                <p className="font-bold text-white">{tier === 5 ? '40%' : tier === 4 ? '35%' : '25%'}</p>
                                <p className="text-primary">${(poolData.finalPool * (tier === 5 ? 0.40 : tier === 4 ? 0.35 : 0.25)).toFixed(2)}</p>
                            </div>
                        ))}
                    </div>
                )}
            ...
            <div className="glass-panel p-6 rounded-2xl border-white/10 space-y-6">
                <h3 className="font-semibold text-lg border-b border-white/5 pb-3">Configure New Draw</h3>
                ...
                <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                        {MODES.map(m => (
                            ...
                            >
                                <m.icon className={`w-5 h-5 mb-2 ${mode === m.value ? 'text-primary' : 'text-muted-foreground'}`} />
                                <p className={`text-sm font-semibold ${mode === m.value ? 'text-primary' : 'text-white'}`}>{m.label}</p>
                ...
                >
                    ...
                    {loading ? 'Generating...' : `Simulate ${MONTHS[drawMonth - 1]} ${drawYear} Draw`}
                </button>
            </div>
            ...
            <div className="space-y-4">
                ...
                {activeDraws.map(draw => {
                    ...
                    return (
                        <div key={draw.id} className="glass-panel p-5 rounded-xl flex flex-col md:flex-row justify-between gap-4 border-white/10">
                            <div className="space-y-2">
                                <div className="flex items-center gap-3 flex-wrap">
                                    <p className="font-bold text-lg">{monthName} {draw.draw_year}</p>
                                    <span className={`text-xs px-2 py-0.5 rounded-full border font-semibold uppercase ${
                                        draw.status === 'published' ? 'bg-primary/20 text-primary border-primary/30'
                                        : 'bg-yellow-500/20 text-yellow-400 border-yellow-500/30'
                                    }`}>
                                        {draw.status}
                                    </span>
                                    <span className="text-xs text-muted-foreground bg-white/5 px-2 py-0.5 rounded">{draw.mode?.replace('_', ' ')}</span>
                                </div>

                                <div className="flex gap-2">
                                    {(draw.winning_numbers || []).map((n: number, i: number) => (
                                        <span key={i} className="w-8 h-8 rounded-full bg-white/10 border border-white/20 flex items-center justify-center text-sm font-bold text-primary">
                                            {n}
                                        </span>
                                    ))}
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
                                    <span>Pool: <span className="text-white font-semibold">${Number(draw.total_pool || 0).toFixed(2)}</span></span>
                                    {draw.rollover_amount > 0 && (
                                        <span className="text-yellow-400">↑ Incl. ${Number(draw.rollover_amount).toFixed(2)} rollover</span>
                                    )}
                                    {draw.jackpot_carryover > 0 && draw.status === 'published' && (
                                        <span className="text-orange-400">→ ${ Number(draw.jackpot_carryover).toFixed(2)} rolling into next month</span>
                                    )}
                                </div>
                            </div>
