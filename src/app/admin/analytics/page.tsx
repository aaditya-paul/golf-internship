...
export default async function AdminAnalyticsPage() {
  ...
  const { data: draws } = await supabase
    .from("draws")
    .select("total_pool, status");
  const totalPoolAllTime =
    draws?.reduce(
      (acc: number, curr: any) => acc + (curr.total_pool || 0),
      0,
    ) || 0;
  ...
  const { data: allWinners } = await supabase
    .from("draw_winners")
    .select("tier, prize_amount, status");
  let match5 = 0,
    match4 = 0,
    match3 = 0,
    totalPaidOut = 0;
  ...
  const charityDonationTotals: Record<
    string,
    { name: string; total: number; users: number }
  > = {};
  donationRows.forEach((row) => {
    const key = row.charityName;
    if (!charityDonationTotals[key])
      charityDonationTotals[key] = { name: key, total: 0, users: 0 };
    charityDonationTotals[key].total += row.monthlyDonation;
    charityDonationTotals[key].users++;
  });
  const charityTotals = Object.values(charityDonationTotals).sort(
    (a, b) => b.total - a.total,
  );
  const totalCharityMRR = donationRows.reduce(
    (s, r) => s + r.monthlyDonation,
    0,
  );
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Platform-wide metrics and financial performance.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="glass-panel p-6 rounded-2xl border-white/10 flex flex-col justify-between">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            MRR: ${((activeSubs || 0) * monthlyAmount).toFixed(2)}
          </p>
        </div>
        <div className="glass-panel p-6 rounded-2xl border-white/10 flex flex-col justify-between">
          ...
          <div className="space-y-3">
            {charityTotals.length === 0 ? (
              ...
              charityTotals.map((c) => (
                <div
                  key={c.name}
                  className="flex justify-between items-center bg-black/30 px-4 py-3 border border-white/5 rounded-xl"
                >
                  <div>
                    <span className="text-sm font-medium">{c.name}</span>
                    <p className="text-xs text-muted-foreground">
                      {c.users} donor{c.users !== 1 ? "s" : ""}
                    </p>
                  </div>
                  <span className="text-primary font-bold">
                    ${c.total.toFixed(2)}/mo
                  </span>
                </div>
              ))
            )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-white/5">
                {donationRows.map((row, i) => (
                  <tr key={i} className="hover:bg-white/5 transition-colors">
                    <td className="py-3 pr-4 text-white/80 max-w-55 truncate">
                      {row.email}
                    </td>
              ...
              <tfoot>
                <tr className="border-t border-white/10">
                  <td
                    colSpan={3}
                    className="pt-3 text-right text-muted-foreground font-semibold"
                  >
                    Total This Month
                  </td>
                  <td className="pt-3 text-right font-bold text-primary text-base">
                    ${totalCharityMRR.toFixed(2)}
                  </td>
                </tr>
              </tfoot>
            </table>
