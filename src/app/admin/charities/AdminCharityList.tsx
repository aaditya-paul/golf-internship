...
export default function AdminCharityList({
  initialCharities,
}: {
  ...
  async function handleDelete(id: string) {
    if (
      !confirm(
        "Are you sure you want to delete this charity? Active users tracking it might be affected.",
      )
    )
      return;
    setLoading(true);
        ...
        <div className="glass p-4 sm:p-6 rounded-2xl border border-white/20 relative">
          ...
          <form action={handleAdd} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={loading}
              className="bg-primary text-black font-semibold px-6 py-2 rounded-lg neon-button"
            >
              Save Charity
            </button>
          </form>
        ...
        {initialCharities.map((charity) => (
          ...
          >
            {editingId === charity.id ? (
              <form
                action={(fd) => handleEdit(charity.id, fd)}
                className="space-y-4"
              >
                <input
                  name="name"
                  ...
                  required
                  className="w-full bg-black/50 border border-white/10 rounded-lg px-4 py-2 text-sm"
                />
                <textarea
                  name="description"
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <button
                    type="submit"
                    disabled={loading}
                    className="bg-primary text-black text-sm font-semibold px-4 py-2 rounded-lg"
                  >
                    Update
                  </button>
                  <button
